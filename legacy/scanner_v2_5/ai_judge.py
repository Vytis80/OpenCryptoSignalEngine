...
PROMPT_VERSION = "ai-judge-v1-v25-usage1"
...
class AIJudge:
    """Observational GPT-OSS reviewer for V2.5 EXECUTE signals only.

    Safety invariant: no exchange/bridge access and no mutation of trading state.
    """
    ...
    def _input(self, a, shadow=None, smart=None):
        shadow_payload = None
        if shadow is not None:
            shadow_payload = {
                "status": getattr(shadow, "status", "NO_DATA"),
                "would_block": bool(getattr(shadow, "would_block", False)),
                "estimated_cost_r": round(float(getattr(shadow, "estimated_cost_r", 0.0)), 4),
        ...
        if smart is not None:
            smart_payload = {
                "verdict": getattr(smart, "verdict", "NO_DATA"),
                "score": round(float(getattr(smart, "score", 0.0)), 2),
                "setup_family": getattr(smart, "setup_family", "UNKNOWN"),
                "regime": getattr(smart, "regime", "TRANSITION"),
                "reasons": list(getattr(smart, "reasons", []) or [])[:3],
            }

        atr_pct = (float(getattr(a, "atr_5m", 0.0)) / float(a.price) * 100.0) if float(a.price or 0) > 0 else 0.0
        return {
            "scanner_version": "V2.5.1",
            "symbol": a.inst_id,
            "side": a.side,
            ...
            "structure": {
                ...
                "market_regime": getattr(a, "market_regime", "TRANSITION"),
                "regime_strength": round(float(getattr(a, "regime_strength", 0.0)), 3),
                "direction_margin": round(float(getattr(a, "direction_margin", 0.0)), 3),
            },
            "market_context": {
                "btc": a.btc_context,
                "eth": a.eth_context,
                "relative_strength_vs_btc_pct": round(float(a.relative_strength), 4),
                "funding_rate": round(float(getattr(a, "funding_rate", 0.0)), 7),
                "oi_delta_pct": round(float(getattr(a, "oi_delta_pct", 0.0)), 4),
            },
            "execution_quality": {
                "spread_pct": round(float(a.spread_pct), 5),
                "volume_ratio_5m": round(float(a.volume_ratio_5m), 4),
                "atr_5m_pct": round(atr_pct, 4),
                "stop_pct": round(float(getattr(a, "stop_pct", 0.0)), 4),
                "estimated_cost_r": round(float(getattr(a, "estimated_cost_r", 0.0)), 4),
                "analysis_delay_sec": round(float(getattr(a, "analysis_delay_sec", 0.0)), 1),
                "fresh": bool(a.fresh),
                "too_late": bool(a.too_late),
            },
            "scanner_reasons": list(a.reasons or [])[:7],
            "scanner_warnings": list(a.blocks or [])[:5],
            "legacy_shadow": shadow_payload,
            "smart_shadow": smart_payload,
        }
    ...
    async def assess(self, a, shadow=None, smart=None) -> AIJudgement:
        ...
        system = (
            ...
            "your verdict, NOT probability of profit. Legacy Shadow and SMART Shadow are only "
            "additional evidence; do not blindly copy their verdicts. The deterministic risk engine "
            "is authoritative: never propose changing entry, SL, TP, leverage, size, or execution. "
            "Keep summary and list items concise."
        )
        payload = {
            "model": self.cfg.ai_judge_model,
            "messages": [
                {"role": "system", "content": system},
                {"role": "user", "content": json.dumps(self._input(a, shadow, smart), ensure_ascii=False, separators=(",", ":"))},
            ],
