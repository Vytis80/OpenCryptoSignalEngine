...
def shadow_assess(a,c5,c1m,cfg):
    """Risk analytics that never mutate the scanner's TradeAnalysis.

    The scanner still emits EXECUTE. In the integrated system AutoTrader may
    route WOULD_BLOCK to manual approval, so these checks must be economically
    explainable and observable rather than fitted to a score alone.
    """
    now=time.time();reasons=[];critical=[];atr=max(a.atr_5m,1e-12)
    stop_pct=(abs(a.price-a.sl)/a.price*100.0) if a.price>0 else 0.0
    estimated_cost_r=(2.0*cfg.shadow_estimated_one_way_cost_pct/stop_pct) if stop_pct>0 else float('inf')
    if estimated_cost_r>cfg.shadow_max_estimated_cost_r:
        critical.append(
            f'estimated entry+exit cost is high ({estimated_cost_r:.2f}R; '
            f'stop {stop_pct:.2f}%)'
        )
    if cfg.shadow_manual_gate_all_shorts and a.side=='SHORT':
        critical.append('SHORT is configured for manual Demo approval pending forward validation')
    confirmed=[x for x in c5 if x.confirm]
    recent=confirmed[-max(3,cfg.shadow_swing_lookback):]
    if not recent:
        reasons=['not enough 5m candles']+critical
        would=bool(critical)
        return ShadowAssessment(
            a.inst_id,would,'WOULD_BLOCK' if would else 'NO_DATA',a.sl,0.0,0.0,now,reasons,
    ...
    would=bool(critical)
    return ShadowAssessment(
        a.inst_id,would,'WOULD_BLOCK' if would else 'PASS',suggested,sl_atr,mom,now,
        reasons or ['shadow filters see no extra risk'],estimated_cost_r=estimated_cost_r,
    )
